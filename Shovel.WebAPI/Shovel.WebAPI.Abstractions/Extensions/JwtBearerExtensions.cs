...
namespace Shovel.WebAPI.Abstractions.Extensions
{
    public static class JwtBearerExtensions
    {
        ...
        {
            var expire = configuration.GetSection("Jwt:Expire").Value;

            return new JwtSecurityToken(
                configuration["Jwt:Issuer"],
                configuration["Jwt:Audience"],
                claims,
                expires: DateTime.UtcNow.AddMinutes(Convert.ToInt32(expire)),
        ...
        public static JwtSecurityToken CreateToken(this IConfiguration configuration, List<Claim> authClaims)
        {
            ...
            var tokenValidityInMinutes = configuration.GetSection("Jwt:TokenValidityInMinutes").Value;

            var token = new JwtSecurityToken(
                issuer: configuration["Jwt:Issuer"],
                audience: configuration["Jwt:Audience"],
                expires: DateTime.Now.AddMinutes(Convert.ToInt32(tokenValidityInMinutes)),
                claims: authClaims,
